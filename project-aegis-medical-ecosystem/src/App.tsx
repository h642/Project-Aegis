...
import { SecurityCaseManager } from './components/SecurityCaseManager';
import { ApiSchemaExplorerView } from './components/ApiSchemaExplorerView';
import { PatientProfileModal } from './components/PatientProfileModal';
import { SupabaseAuthModal } from './components/SupabaseAuthModal';
import { supabase, saveSupabaseTelemetryLog, saveSupabaseTriageRecord, saveSupabaseMedicalProfile } from './lib/supabase';
...
export default function App() {
  ...
  const executeEngineTrigger = async (
    ...
    try {
      ...
      // Log to Supabase telemetry_logs table asynchronously
      const activeTelemetry = customTelemetry || telemetry;
      saveSupabaseTelemetryLog(patient.id, activeTelemetry).catch((e) =>
        console.warn('Telemetry log Supabase insert note:', e)
      );

      // If emergency event, log triage record to Supabase triage_history table
      if (trigger === 'EMERGENCY_EVENT' && data.triage_summary) {
        saveSupabaseTriageRecord(patient.id, data as EmergencyTriageResponse).catch((e) =>
          console.warn('Triage record Supabase insert note:', e)
        );

        if (data.triage_summary?.emergency_level === 'CRITICAL_EMERGENCY_DISPATCH') {
